...
public class Wave extends Condition {
	...
	public Wave(AdvancedRobot robot, Enemy enemyInfo, Point2D center, double firingPower) {
		_robot = robot;
		_enemyInfo = enemyInfo;
		_center = center;
		_velocity = GeometryUtils.bulletVelocity(firingPower);
		_bearing = enemyInfo.bearing;
		_lateralDirection = enemyInfo.direction;
	}
 
	public boolean test() {
		_distanceTraveled += _velocity;
		_enemyInfo = Memory.enemies.get(_enemyInfo.name);
		if (_enemyInfo == null) {
